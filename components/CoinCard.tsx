...
export default function CoinCard({ coinPackage, onSelect }: CoinCardProps) {
  ...
  return (
    <div
      className={`relative group bg-[#0f141f] rounded-2xl border p-5 sm:p-6 transition-all duration-300 flex flex-col justify-between overflow-hidden ${
        isPopular || isBestValue
          ? 'border-amber-500/60 shadow-xl shadow-amber-500/10 hover:border-amber-400'
          : 'border-slate-800 hover:border-slate-700 hover:bg-[#141b29]'
      }`}
    >
      ...
      {coinPackage.badge && (
        <div className="absolute top-3 right-3">
          <span className="inline-flex items-center gap-1 bg-amber-950/90 text-amber-300 border border-amber-500/40 text-[11px] font-black uppercase px-2.5 py-1 rounded-full shadow-sm">
      ...
      <div>
        {/* Coin Icon Visual */}
        <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-amber-500 via-yellow-400 to-amber-200 flex items-center justify-center shadow-lg shadow-amber-500/30 mb-4 group-hover:scale-110 transition-transform">
          <Coins className="w-7 h-7 text-black" strokeWidth={2.2} />
        </div>

        {/* Amount */}
        <div className="mb-2">
          <h3 className="text-2xl sm:text-3xl font-black text-white tracking-tight flex items-baseline gap-1.5">
          ...
          {coinPackage.bonusCoins ? (
            <span className="text-xs font-bold text-emerald-400 bg-emerald-950/40 border border-emerald-500/30 px-2 py-0.5 rounded inline-block mt-1">
      ...
      <div className="pt-4 border-t border-slate-800/80 flex items-center justify-between gap-3 mt-2">
        ...
        <button
          ...
          className="flex-1 max-w-[140px] py-3 px-4 rounded-xl bg-gradient-to-r from-amber-400 via-amber-300 to-yellow-400 hover:from-amber-300 hover:to-yellow-300 text-black font-black text-xs sm:text-sm uppercase tracking-wider shadow-lg shadow-amber-500/20 hover:scale-[1.02] active:scale-95 transition-all text-center flex items-center justify-center cursor-pointer"
        >
          IIBSO HADDA
        </button>
      </div>
